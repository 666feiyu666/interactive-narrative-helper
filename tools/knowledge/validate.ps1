...
$formalReleaseBuilder = Join-Path $repositoryRoot 'tools\knowledge\build-track-a-educational-design-knowledge-v1.mjs'
...
$formalReleaseTests = Join-Path $repositoryRoot 'agent\educational-design-helper\tests\knowledge-release.test.mjs'

foreach ($script in @($rulesV10, $rulesV11, $workbookPathContract, $builderV10, $builderV11, $formalReleaseBuilder, $profile, $testsV10, $testsV11, $workbookPathTests, $releaseTests, $formalReleaseTests)) {
    & node --check $script
...
& node --test $testsV10 $testsV11 $workbookPathTests $releaseTests $formalReleaseTests
if ($LASTEXITCODE -ne 0) {
