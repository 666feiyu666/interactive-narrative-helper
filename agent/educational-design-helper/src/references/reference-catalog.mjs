...
export async function loadReferenceCatalog(validators, release) {
  ...
  if (catalog.knowledge_release_id !== release.manifest.knowledge_release_id) {
    throw new Error("Reference catalog and knowledge release do not match.");
  }
  ...
  const designCardIds = new Set(release.designCards.map((card) => card.knowledge_id));
